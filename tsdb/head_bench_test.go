...
func BenchmarkHeadShardedAllPostings(b *testing.B) {
	...
	b.Run("shardCount=001", func(b *testing.B) {
		strategies := []struct {
			name string
			fn   func() index.Postings
		}{
			{"api", func() index.Postings { return h.ShardedAllPostings(0, 1) }},
			{"all-postings", h.postings.All},
			{"fullscan", func() index.Postings { return shardedAllPostingsViaStripeScanForBenchmark(h, 0, 1) }},
		}
